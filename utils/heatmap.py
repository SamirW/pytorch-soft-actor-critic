...
def get_observations(agent_poses):
    ...
    for i, agent_pos in enumerate(agent_poses):
        ...
        obs_n.append(np.concatenate([np.array([0, 0])] + [agent_pos] + entity_pos + other_pos + comm + [[0.]]))

    return np.array([obs_n])

def add_arrows(axes, delta_dict, arrow_color="black", rescale=False):
...
def heatmap(sac, title="Agent Policies", save=False, continuous=False):
            ...
            for x in np.linspace(-1, 1, num_arrows):
                for y in np.linspace(-1, 1, num_arrows):
                    ...
                    torch_obs = [Variable(torch.Tensor(np.vstack(obs[:, k])),
                                          requires_grad=False)
                                 for k in range(sac.nagents)]
                    torch_agent_actions = sac.step(torch_obs, eval=True)
                    action = torch_agent_actions[i].data.numpy()

                    obs = [o.repeat(2,1) for o in torch_obs]
                    act = [a.repeat(2,1) for a in torch_agent_actions]

                    qf_in = torch.cat((*obs, *act), dim=1)
                    qf_out_1, qf_out_2 = sac.agents[i].critic(qf_in)
                    delta_dict[tuple(agent_pos)] = action
                    val_dict[tuple(agent_pos)] = torch.min(qf_out_1.mean(), qf_out_2.mean())
