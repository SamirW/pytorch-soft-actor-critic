        ...
        if len(replay_buffer) > args.batch_size:
            ...
            for i in range(args.updates_per_step):
                for a_i in range(sac.nagents):
                    ...
                    value_loss, critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = sac.update_parameters(a_i, sample, updates)

                    writer.add_scalar('agent_{}/loss/value'.format(a_i), value_loss, updates)
                    writer.add_scalar('agent_{}/loss/critic_1'.format(a_i), critic_1_loss, updates)
                    writer.add_scalar('agent_{}/loss/critic_2'.format(a_i), critic_2_loss, updates)
                    writer.add_scalar('agent_{}/loss/policy'.format(a_i), policy_loss, updates)
                    writer.add_scalar('agent_{}/loss/entropy_loss'.format(a_i), ent_loss, updates)
                    writer.add_scalar('agent_{}/entropy_temprature/alpha'.format(a_i), alpha, updates)
                updates += 1    
